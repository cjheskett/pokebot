...
function textbox.name(letter, randomize)
	local inputting = pb_memory.value("menu", "text_input") == 240
	if (inputting) then
		if (pb_memory.value("menu", "text_length") > 0) then
			input.press("Start")
			return true
		end
		local lidx
		...
		local crow = pb_memory.value("menu", "input_row")
		local drow = math.ceil(lidx / 9)
		if (menu.balance(crow, drow, true, 6, true)) then
			local ccol = math.floor(pb_memory.value("menu", "column") / 2)
		...
		if (pb_memory.raw(0x10B7) == 3) then
			input.press("A", 2)
...
function textbox.isActive()
	return pb_memory.value("game", "textbox") == 1
end
