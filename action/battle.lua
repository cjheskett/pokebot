...
local function recover()
	...
	if (pb_memory.value("battle", "paralyzed") == 64) then
...
local function openBattleMenu()
	if (pb_memory.value("battle", "text") == 1) then
		input.cancel()
		return false
	end
	...
	elseif (battleMenu == 94) then
		local rowSelected = pb_memory.value("menu", "row")
		if (col == 9) then
...
local function attack(attackIndex)
	if (pb_memory.double("battle", "opponent_hp") < 1) then
		input.cancel()
...
function battle.isActive()
	return pb_memory.value("game", "battle") > 0
end

function battle.isTrainer()
	local battleType = pb_memory.value("game", "battle")
...
function battle.opponent()
	return pokemon.getName(pb_memory.value("battle", "opponent_id"))
end

function battle.run()
	if (pb_memory.double("battle", "opponent_hp") < 1) then
		input.cancel()
	elseif (pb_memory.value("battle", "menu") ~= 94) then
		if (pb_memory.value("menu", "text_length") == 127) then
			input.press("B")
		else
			input.cancel()
		end
	elseif (textbox.handle()) then
		local selected = pb_memory.value("menu", "selection")
...
function battle.handleWild()
	if (pb_memory.value("game", "battle") ~= 1) then
		return true
	end
	battle.run()
end
...
function battle.fight(move, isNumber, skipBuffs)
	if (move) then
		...
		if (move) then
			attack(move.midx)
		elseif (pb_memory.value("menu", "text_length") == 127) then
			--print("Faito B!")
			input.press("B")
		else
			input.cancel()
...
function movePP(name)
	...
	return pb_memory.raw(0xD02C + midx)
end
battle.pp = movePP

function battle.automate(moveName, skipBuffs)
	if (not recover()) then
		local state = pb_memory.value("game", "battle")
		if (state == 0) then
			input.cancel()
		else
