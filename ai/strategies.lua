...
local function opponentDamaged(factor)
	...
	return pb_memory.double("battle", "opponent_hp") * factor < pb_memory.double("battle", "opponent_max_hp")
end
...
local function buffTo(buff, defLevel)
	if (battle.isActive()) then
		canProgress = true
		local forced
		if (pb_memory.double("battle", "opponent_defense") > defLevel) then
			forced = buff
		end
...
local function dodgeUp(npc, sx, sy, dodge, offset)
	...
	elseif (px == sx or px == dodge) then
		if (px - pb_memory.raw(npc) == offset) then
			if (px == sx) then
				wx = dodge
			else
				wx = sx
...
local function dodgeH(options)
	...
	elseif (py == options.sy or py == options.dodge) then
		if (py - pb_memory.raw(options.npc) == options.offset) then
...
local function completedSkillFor(data)
	if (data.map) then
		if (data.map ~= pb_memory.value("game", "map")) then
			return true
		end
	...
	elseif (data.done) then
		if (pb_memory.raw(data.done) > (data.val or 0)) then
			return true
		end
...
local function isPrepared(...)
	...
	for i,itemState in ipairs(tries) do
		...
		if (count > 0 and count == inventory.count(name)) then
			...
			if (not opp or opp == pb_memory.value("battle", "opponent_id")) then
				return false
			end
		end
	end
...
local function nidoranDSum(disabled)
	...
	if (not disabled and tries == nil) then
		...
		local opLevel = pb_memory.value("battle", "opponent_level")
...
strategyFunctions = {
	...
	reportFrames = function()
		...
		local repels = pb_memory.value("player", "repel")
		if (repels > 0) then
			print("S "..repels)
		end
...
-- Global
	...
	fly = function(data)
		if (pb_memory.value("game", "map") == data.map) then
			return true
		end
		local cities = {
			pallet = {62, "Up"},
		...
		local main = pb_memory.value("menu", "main")
		if (main == 228) then
			local currentFly = pb_memory.raw(0x1FEF)
	...
	bicycle = function()
		if (pb_memory.raw(0x1700) == 1) then
...
-- Route
	...
	fightBulbasaur = function()
		...
		if (battle.isActive() and pb_memory.double("battle", "opponent_hp") > 0 and resetTime(2.15, "kill Bulbasaur")) then
			return true
		end
	...
	catchNidoran = function()
		...
		if (battle.isActive()) then
			...
			if (pb_memory.value("menu", "text_input") == 240) then
				textbox.name()
			elseif (pb_memory.value("battle", "menu") == 95) then
				if (isNidoran) then
					input.press("A")
				else
					input.cancel()
...
-- 1: NIDORAN
	...
	fightWeedle = function()
		if (battle.isTrainer()) then
			...
			if (squirtleOut and pb_memory.value("battle", "our_status") > 0 and not inventory.contains("antidote")) then
			...
			elseif (squirtleOut) then
				...
				local forced
				if (peck and peck.damage and peck.damage + 1 >= pb_memory.double("battle", "opponent_hp")) then
					forced = "growl"
				end
				battle.fight(forced)
			end
		...
		elseif (menu.pause()) then
			...
			elseif (inventory.contains("potion") and pokemon.info("squirtle", "hp") < 15) then
				...
				elseif (main == 103) then
					if (pb_memory.value("menu", "selection_mode") == 1) then
	...
	fightBrock = function()
		...
		if (battle.isActive()) then
			...
			elseif (turnsToDie and turnsToDie < 2 and inventory.contains("potion")) then
				...
				local battleMenu = pb_memory.value("battle", "menu")
				local bideTurns = pb_memory.value("battle", "opponent_bide")
				if (battleMenu == 95 and menu.getCol() == 1) then
					input.press("A")
				elseif (bideTurns > 0) then
					local onixHP = pb_memory.double("battle", "opponent_hp")
...
-- 2: BROCK
	...
	battleModeSet = function()
		if (pb_memory.value("setting", "battle_style") == 10) then
			if (menu.close()) then
				return true
			end
		elseif (menu.pause()) then
	...
	fightMetapod = function()
		if (battle.isActive()) then
			canProgress = true
			if (pb_memory.double("battle", "opponent_hp") > 0 and pokemon.isOpponent("metapod")) then
				return true
			end
			battle.automate()
		elseif (canProgress) then
	...
	catchFlierBackup = function()
		...
		if (battle.isActive()) then
			if (pb_memory.double("battle", "our_hp") == 0) then
				if (pokemon.info("squirtle", "hp") == 0) then
					strategies.canDie = false
				elseif (utils.onPokemonSelect(pb_memory.value("battle", "menu"))) then
					menu.select(pokemon.indexOf("squirtle"), true)
				else
					input.press("A")
				end
...
-- 3: ROUTE 3
	...
	evolveNidorino = function()
		...
		if (battle.isActive()) then
			tries = 0
			canProgress = true
			if (pb_memory.double("battle", "opponent_hp") == 0) then
				input.press("A")
			else
				battle.automate()
			end
...
-- 6: MISTY
	...
	catchOddish = function()
		...
		local battleValue = pb_memory.value("game", "battle")
		local px, py = player.position()
	...
	trashcans = function()
		local progress = pb_memory.value("progress", "trashcans")
		if (textbox.isActive()) then
			if (not canProgress) then
				if (progress < 2) then
					tries = tries + 1
...
-- 7: SURGE
	...
	swapBicycle = function()
		...
		if (main == 128) then
			if (menu.getCol() ~= 5) then
				...
				local selection = pb_memory.value("menu", "selection_mode")
				if (selection == 0) then
	...
	giveWater = function()
		...
		if (textbox.isActive()) then
			...
			local cx, cy = pb_memory.raw(0x0223) - 3, pb_memory.raw(0x0222) - 3
			local px, py = player.position()
		...
		if (battle.isActive()) then
			...
			if (pokemon.isDeployed("nidoking")) then
				if (pokemon.info("nidoking", "hp") == 0) then
					if (utils.onPokemonSelect(pb_memory.value("battle", "menu"))) then
...
-- 8: POKÉFLUTE
	...
	playPokeflute = function()
		...
		if (pb_memory.value("battle", "menu") == 95) then
			input.press("A")
		elseif (menu.pause()) then
...
-- 10: KOGA
	...
	dodgeGirl = function()
		local gx, gy = pb_memory.raw(0x0223) - 5, pb_memory.raw(0x0222)
		local px, py = player.position()
		if (py > gy) then
			if (px > 3) then
				px = 3
...
-- 15: GIOVANNI
	...
	ether = function(data)
		...
		if (tempDir and completedMenuFor(data)) then
			...
			if (pb_memory.value("menu", "main") == 144 and menu.getCol() == 5) then
				if (pb_memory.value("battle", "menu") ~= 95) then
	...
	depositPokemon = function()
		...
		if (pb_memory.value("player", "party_size") == toSize) then
			if (menu.close()) then
				return true
			end
		else
			if (not textbox.isActive()) then
				...
				local pc = pb_memory.value("menu", "size")
				if (pb_memory.value("battle", "menu") ~= 95 and (pc == 2 or pc == 4)) then
	...
	champion = function()
		if (canProgress) then
			...
			if (tries == 0) then
				...
				if (strategies.seed) then
					print(pb_memory.value("game", "frames").." frames, with seed "..strategies.seed)
					print("Please save this seed number to share, if you would like proof of your run!")
				end
			end
			tries = tries + 1
		elseif (pb_memory.value("menu", "shop_current") == 252) then
