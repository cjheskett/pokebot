...
local function isNewFight()
	if (fightEncounter < encounters and pb_memory.double("battle", "opponent_hp") == pb_memory.double("battle", "opponent_max_hp")) then
...
function control.shouldFight()
	...
	if (expTotal < minExp) then
		local oid = pb_memory.value("battle", "opponent_id")
		local olvl = pb_memory.value("battle", "opponent_level")
		for i,p in ipairs(shouldFight) do
			if (oid == pokemon.getID(p.name) and (not p.lvl or utils.match(olvl, p.lvl))) then
				if (oneHits) then
					...
					if (move and move.maxDamage * 0.925 < pb_memory.double("battle", "opponent_hp")) then
						return false
					end
				end
				return true
...
function control.canCatch(partySize)
	if (not partySize) then
		partySize = pb_memory.value("player", "party_size")
	end
...
function control.shouldCatch(partySize)
	if (maxEncounters and encounters > maxEncounters) then
		...
		if (not extraCount or encounters > maxEncounters + 1) then
			require("ai.strategies").reset("Too many encounters homie", encounters)
			--print("I woulda reset for encounters")
			return false
		end
	end
	...
	for i,poke in ipairs(shouldCatch) do
		if (oid == pokemon.getID(poke.name) and not pokemon.inParty(poke.name, poke.alt)) then
			if (not poke.lvl or utils.match(pb_memory.value("battle", "opponent_level"), poke.lvl)) then
				local penultimate = poke.hp and pb_memory.double("battle", "opponent_hp") > poke.hp
				if (penultimate) then
