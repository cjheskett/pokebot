...
function inventory.indexOf(name)
	...
	for i=0,19 do
		...
		if (pb_memory.raw(iidx) == searchID) then
			return i
		end
	end
	return -1
...
function inventory.count(name)
	...
	if (index ~= -1) then
		return pb_memory.raw(ITEM_BASE + index * 2 + 1)
	end
	return 0
end
...
function inventory.use(item, poke, midfight)
	...
	local main = pb_memory.value("menu", "main")
	local column = menu.getCol()
	--bridge.chat("inventory.use()".." main="..main.." column="..column)
	if (main == 144) then
