...
local function index(index, offset)
	...
	local value = pb_memory.raw(address)
	if (double) then
		value = value + pb_memory.raw(address + 1)
	end
	return value
end
pokemon.index = index
...
function pokemon.battleMove(name)
	...
	for i=1,4 do
		if (mid == pb_memory.raw(0xD01B + i)) then
			return i
		end
	end
end
...
function pokemon.moveIndex(move, pokemon)
	...
	for i=1,4 do
		if (mid == pb_memory.raw(address + i)) then
			return i
		end
	end
end
...
function pokemon.forMove(move)
	...
	for i=0,5 do
		...
		for j=8,11 do
			if (pb_memory.raw(address + j) == moveID) then
				return i
			end
		end
	end
...
function pokemon.updateParty()
	local partySize = pb_memory.value("player", "party_size")
...
function pokemon.isDeployed(...)
	for i,name in ipairs(arg) do
		if (pb_memory.value("battle", "our_id") == pokeIDs[name]) then
			return name
		end
	end
end
...
function pokemon.isEvolving()
	return pb_memory.value("menu", "pokemon") == 144
end

function pokemon.getExp()
	return pb_memory.raw(0xD17A) * 256 + pb_memory.raw(0xD17B)
end
...
function pokemon.use(move)
	...
	elseif (main == 128) then
		...
		elseif (column == 10 or column == 12) then
			local midx = 0
			local menuSize = pb_memory.value("menu", "size")
			if (menuSize == 4) then
				if (move == "dig") then
					midx = 1
